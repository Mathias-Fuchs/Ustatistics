...
double gamma(const gsl_matrix * data, const gsl_vector * response) {
	...
	gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1.0, Xlearn, Xlearn, 0.0, CC);
	gsl_blas_dgemv(CblasTrans, 1.0, Xlearn, ylearn, 0.0, DD);
	inv2inPlace(CC, ii);


	gsl_blas_dsymv(CblasUpper, 1.0, ii, DD, 0, EE);
	...
	double ypredicted;
	gsl_blas_ddot(Xtest, EE, &ypredicted);
...
int main(int argc, char ** argv) {
	...
	size_t n = 103;
	size_t p = 3;
	int B = 1e6; // number of resample in each iteration
	int seed = 1234; // random seed
	int g = 40; // learning set size

	CC = gsl_matrix_alloc(3, 3);
	DD = gsl_vector_alloc(3);
	EE = gsl_vector_alloc(3);
	ii = gsl_matrix_alloc(3, 3);
	...
	printf("leave-p-out estimator: %f estimator for thetasquared: %f estimator for its variance: %f\n", lpo, t2, gsl_pow_2(lpo) - t2);
