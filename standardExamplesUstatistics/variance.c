...
double kern(const gsl_matrix* data) {
	double pp = gsl_matrix_get(data, 0, 0) - gsl_matrix_get(data, 1, 0);
	return pp * pp / 2.0;
}
...
int main() {
	...
	double estimatedMean = U(
		data, B, 2, r, kern, &computationConfIntLower, &computationConfIntUpper, &thetaConfIntLower, &thetaConfIntUpper, NULL);
